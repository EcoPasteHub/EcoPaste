...
- **基于 Tauri 构建**：使应用更加小巧精致，资源占用更少，并适配多个平台。
- **应用常驻后台**：通过自定义快捷键一键唤醒，节省时间，提高效率。
- **数据隐私与安全**：所有剪切板内容均在本地存储，确保数据隐私和安全。
- **内容多类型支持**：支持纯文本、富文本、HTML、图片和文件等类型内容的操作。
- **智能分组**：根据剪切板内容类型自动分组，便于高效管理。
- **收藏功能**：支持收藏剪切板内容，便于快速访问和管理重要信息。
- **快捷搜索**：轻松找到剪切板上的任何内容。
- **图片 OCR 功能**：内置系统 OCR 功能，支持文本和二维码识别。
- **右键菜单**：提供丰富的右键菜单操作，提高操作便捷性。
- **自定义配置**：提供丰富的自定义配置选项，满足个性化需求。
- **导入导出**：支持导出和导入配置及剪切板内容，便于数据备份与迁移。
- **键盘操作**：支持键盘快捷键快速唤醒及复制粘贴，提高操作效率。
- **自动更新**：软件支持自动更新和手动检查更新功能，确保始终保持最新版本。
- **更多功能**：更多好玩的功能，期待与您一起探索...
