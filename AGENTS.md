...
- 以 `package.json` 的 `version` 为准：只要版本仍是 `0.6.0-beta.3`，本仓库就视为**未发版开发环境**，可以直接修改数据结构、文件布局、配置格式与默认值，不需要写迁移、兼容、回退或迭代代码。
- 只有当 `package.json` 的 `version` 从 `0.6.0-beta.3` 变更为其它版本后，才开始把已有用户数据视为需要兼容的发布数据；届时再按实际版本差异补迁移与兼容逻辑。
- 若新增旧项目没有的功能、行为、平台能力、文件格式或用户可感知变化，必须同步更新 [CHANGELOG.md](./CHANGELOG.md)；判断是否属于新增内容时，以 `/Users/ayang/Documents/PersonalProject/2024/EcoPaste_bak` 或公开旧仓库为参照。同一能力的多个细节不要拆成多条并列项，优先用一个能力条目承载子项。
...
- migration 只增不改：已合并的迁移文件不要回头修改，新增变更写新文件（**例外**：当 `package.json` 的 `version` 仍是 `0.6.0-beta.3` 时，视为未发版开发期，所有改动直接合并到 `0001_init.sql`，不新增文件，也不写兼容迁移）。
